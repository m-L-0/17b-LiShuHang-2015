    ...
    def get_batch(self, batch_size):
        with tf.name_scope('get_batch'):
            ...
            with tf.Session() as sess:
                ...
                coord.request_stop()
                coord.join(thread)
                lab = np.zeros([batch_size, 34], dtype=int)
...
with tf.Graph().as_default() as T:
    ...
    # 输出层
    w_out = weight_variable([512, 34])
    b_out = bias_variable([34])
...
with tf.Session(graph=T) as sess:
    ...
    test = train_data.get_batch(1000)[1]
    ...
    for count in range(34):
        for i in range(test.shape[0]):
            ...
            if i != 0:
                ...
                if j1 != j:
                    test_temp = test[:i]
                    correct_prediction = np.equal(test_temp[:, 0],
                                                  test_temp[:, 1])
    ...
    # Y = np.zeros(100)
    # X_batch, y_batch = validate_data.get_batch(100)
    # for j in range(36):
    #     k = 0
    #     l = 0
    #     for i in range(100):
