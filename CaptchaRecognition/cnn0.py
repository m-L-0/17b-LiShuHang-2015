from read import gen_captcha_text_and_image
...
def crack_captcha_cnn(w_alpha=0.01, b_alpha=0.1):
    ...
    w_c3 = tf.Variable(w_alpha * tf.random_normal([3, 3, 64, 96]))
    b_c3 = tf.Variable(b_alpha * tf.random_normal([96]))
    conv3 = tf.nn.relu(
        tf.nn.bias_add(
    ...
    w_d = tf.Variable(w_alpha * tf.random_normal([8 * 20 * 96, 1024]))
